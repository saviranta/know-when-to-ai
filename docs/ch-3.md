...
*Illustration 3.1 — The method at a glance. Two Tier-0 checks sit above five Tier-1 gates. The atlas carries a tool palette for each gate; the AI capability atlas is picked only at G3. Loopbacks from G3 back to G2 are cheap and expected.*
...
The first is the **Technochauvinism check** [1]: is this a technical problem at all, or a social problem renamed? Both answers are valid; the check forces the question to be asked before a tool is reached for. Failing it produces *Not an AI problem* — policy, process, or people, not tools.
...
The effect on the method is simple. Everything else waits for the two Tier-0 checks, which can produce two outcomes: *Off-limits to AI* (the Abolition check fails) or *Not an AI problem* (the Technochauvinism check fails). If both checks pass, Tier 1 opens. Two further outcomes are available inside the method: *AI as assistant* (produced at G3 Route when no AI level clears the controls bar for a specific sub-problem — a human holds the decision, the machine supports) and *Autonomous AI* (produced at G3 Route when a level does clear the controls bar — the machine makes the call under the controls designed for it). Four outcomes in all, produced at three places — the two Tier-0 checks, and G3.

[Chapter 4](ch-4.md) carries the full treatment — the four "dressed-as-tech" patterns at the Technochauvinism check, the four axes at the Abolition check, the tools each uses to produce its one-paragraph determination.

A word on G1 Observe. Illustration 1.1 in [Chapter 1](ch-1.md) shows the four answers; Illustration 3.1 shows the five gates. G1 folds what older method traditions split into *framing* (stating the problem in the world, not the brief's version of it) and *diagnosis* (finding the system the symptom lives in) — because both produce one artefact, the situation statement, through one iterative pass. [Chapter 5](ch-5.md) treats the craft of framing and diagnosis together.
...
[Chapter 4](ch-4.md) opens Part 2 with the two Tier-0 checks — Technochauvinism and Abolition — and the tools each uses to produce its one-paragraph determination. [Chapter 5](ch-5.md) picks up G1 Observe; in practice, the framing conversation and the Tier-0 checks usually happen side by side, but the book treats them in separate chapters to keep the work of each visible.
