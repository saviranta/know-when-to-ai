...
The effect on the method is simple. Everything else waits for the two Tier-0 gates, which can produce two outcomes: *Abolish* (Gate 2 fails) or *Redirect* (Gate 1 fails). If both gates pass, Tier 1 opens. Two further outcomes are available inside the method: *Judge* (produced at G3 Route when no AI rung clears the controls bar for this specific problem) and *Proceed* (the default when routing finds a viable placement). Four outcomes in all, produced at three places — Gate 1, Gate 2, and G3.
